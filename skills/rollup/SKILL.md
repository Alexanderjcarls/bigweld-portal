...
RETURN s.name AS scope,
       ...
       [a IN articles WHERE a.updated_ts > datetime() - duration('P30D') | a.title][0..5] AS recent_30d,
       [a IN articles | a.title][0..10] AS sample_titles
```
...
**Table:** Sample titles (up to 10) with their `id` so Alex can drill in.
...
```cypher
MATCH (s:Scope {name: $scope})<-[:IN_SCOPE]-(a:Article)-[:TAGGED]->(t:Tag)
